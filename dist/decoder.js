...
if (!Number.isInteger(inputValue) || inputValue <= 0 || (inputValue & (inputValue - 1)) !== 0) {
    console.error(`Zadaná hodnota ${inputValue} je neplatná! Číslo musí být kladná mocnina 2!`);
    process.exit(1);
}
;
...
const cases = Array.from({ length: inputValue }, (_, i) => {
    ...
    const output = "0".repeat(inputValue - 1 - i) +
        "1" +
        "0".repeat(i);
...
`;
fs_1.default.writeFileSync(`decoder_${inputValue}.vhd`, vhdl);
